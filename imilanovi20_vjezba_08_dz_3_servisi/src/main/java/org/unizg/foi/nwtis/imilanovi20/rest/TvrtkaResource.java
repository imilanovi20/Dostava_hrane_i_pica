...
public class TvrtkaResource {
  ...
  /** Klijent. */
  private HttpClient klijent = HttpClient.newHttpClient();
  ...
  public Response headPosluziteljKrajInfo() {
    try {
      var odgovor = this.tvrtkaInfoKlijent.getPosluziteljKrajInfo();
  ...
  public Response postObracun(Obracun[] obracuni) {
    try (var vezaBP = this.restConfiguration.dajVezu()) {
      ...
      if (status) {
        var odgovor = this.tvrtkaInfoKlijent.getObracunWS();
        if (odgovor.getStatus() != 200) {
          return Response.status(Response.Status.INTERNAL_SERVER_ERROR).build();
        }
        return Response.status(Response.Status.CREATED).build();
